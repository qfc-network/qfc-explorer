...
export default async function LeaderboardPage({
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <div>
        <h1 className="text-lg font-semibold text-slate-900 dark:text-white"><TranslatedText tKey="leaderboard.title" /></h1>
        <p className="mt-1 text-sm text-slate-400"><TranslatedText tKey="leaderboard.description" /></p>
      </div>

      {/* Tabs */}
      <div className="mt-6 flex gap-0 border-b border-slate-200 dark:border-slate-800">
        {tabs.map((t) => (
          <Link
            key={t.key}
          ...
          >
            {t.label}
            {t.count > 0 && (
              <span className="ml-1.5 rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-xs text-slate-400">{t.count}</span>
            )}
          </Link>
        ))}
      </div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-200 dark:divide-slate-800/40">
          {rows.length === 0 ? (
            ...
            return (
              <tr key={r.address} className="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                ...
                <td className="px-4 py-2.5 text-right text-slate-600 dark:text-slate-300">{formatWeiToQfc(r.balance)}</td>
                <td className="px-4 py-2.5">
          ...
          {rows.length === 0 ? (
            ...
            return (
              <tr key={r.address} className="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                ...
                <td className="px-4 py-2.5 text-right text-slate-600 dark:text-slate-300">{formatNumber(r.sent)}</td>
                <td className="px-4 py-2.5 text-right text-slate-600 dark:text-slate-300">{formatNumber(r.received)}</td>
                <td className="px-4 py-2.5 text-right font-medium text-slate-900 dark:text-white">{formatNumber(r.total)}</td>
                <td className="px-4 py-2.5">
                  <div className="h-2 w-32 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
          ...
          {rows.length === 0 ? (
            ...
            return (
              <tr key={r.address} className="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                ...
                <td className="px-4 py-2.5 text-right font-medium text-slate-900 dark:text-white">{formatNumber(r.blocks_produced)}</td>
                <td className="px-4 py-2.5 text-right text-slate-600 dark:text-slate-300">{sharePct}%</td>
                <td className="px-4 py-2.5 text-xs text-slate-400">
                  <Link href={`/blocks/${r.first_block}`} className="hover:text-slate-900 dark:hover:text-white">{r.first_block}</Link>
                  {' — '}
                  <Link href={`/blocks/${r.last_block}`} className="hover:text-slate-900 dark:hover:text-white">{r.last_block}</Link>
                </td>
                <td className="px-4 py-2.5">
                  <div className="h-2 w-32 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
          ...
          {rows.length === 0 ? (
            ...
            return (
              <tr key={r.address} className="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                ...
                <td className="px-4 py-2.5 text-right font-medium text-slate-900 dark:text-white">{formatNumber(r.tx_count)}</td>
                <td className="px-4 py-2.5">
                  <div className="h-2 w-32 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
