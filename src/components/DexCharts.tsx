...
// Mock time-series data — will be replaced with API data
...
const CHARTS: ChartConfig[] = [
  {
    ...
    description: '24h trading volume over time',
    color: '#a855f7',
    type: 'line',
    formatValue: (v) => `$${(v / 1000).toFixed(0)}K`,
    getData: (days) => generateMockSeries(days, 95000, 30000),
  },
  {
    ...
    getData: (days) => generateMockSeries(days, 5200000, 500000),
  },
  {
    title: 'Swap Count',
    ...
    getData: (days) => generateMockSeries(days, 1400, 500),
  },
  {
    title: 'Fee Revenue',
    description: 'Daily trading fees collected (0.3%)',
    color: '#f59e0b',
    type: 'line',
    formatValue: (v) => `$${v.toLocaleString()}`,
    getData: (days) => generateMockSeries(days, 285, 100),
  },
];
...
export default function DexCharts() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {CHARTS.map((chart) => (
          <DexChart key={chart.title} chart={chart} days={days} />
        ))}
      </div>
    </div>
  );
...
function DexChart({ chart, days }: { chart: ChartConfig; days: number }) {
