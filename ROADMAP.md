...
### 1.1 首页重设计
- [ ] 顶部网络概览卡片栏：QFC Price / Market Cap / TPS / Block Time / Gas / Finalized Block
- [ ] 最新区块 + 最新交易双列实时滚动（参考 Etherscan 首页布局）
- [ ] 迷你折线图：14 天交易量趋势 + 活跃地址趋势
- [ ] 搜索框居中突出，支持 placeholder 提示 "Search by Address / Tx Hash / Block / Token"

### 1.2 全局搜索增强
- [ ] 统一搜索框：地址 / TX Hash / Block Height / Block Hash / Token Name / Contract Name
- [ ] 搜索下拉实时建议（debounce 300ms）
- [ ] 搜索历史记录（localStorage）
- [ ] 无结果时显示友好提示 + 相似建议

### 1.3 Header/Footer 优化
- [ ] 固定导航栏：Home / Blockchain / Tokens / AI Inference / Network / More
- [ ] 下拉子菜单：Blockchain → Blocks, Transactions, Pending Txs
- [ ] Footer：链接到 qfc.network、Docs、GitHub、Faucet、社交媒体
...
### 2.1 地址页 Tab 化
- [ ] Overview Tab：余额、QFC 价值估算、Nonce、首次/最后活跃
- [ ] Transactions Tab：分页交易列表（筛选 In/Out/All）
- [ ] Internal Transactions Tab：合约内部调用追踪
- [ ] Token Transfers Tab：ERC-20 转账记录
- [ ] Contract Tab（合约地址）：Code / Read / Write / Events

### 2.2 交易页增强
- [ ] Input Data 解码：已验证合约自动解码函数名+参数
- [ ] Internal Transactions 显示
- [ ] Event Logs 解码（匹配已验证合约 ABI）
- [ ] 交易状态时间线：Pending → Confirmed → Finalized
- [ ] Gas 使用可视化（used / limit 进度条）
...
### 3.1 ERC-20 增强
- [ ] Token 持有者排名页（Top Holders with percentage）
- [ ] Token 详情页：总供应量、持有者数、转账次数、合约信息
- [ ] 地址 Token 余额汇总（Portfolio 视图）
- [ ] Token 转账筛选：按 From / To / Token 过滤

### 3.2 ERC-721 / ERC-1155 NFT 支持
- [ ] NFT 合约识别（Transfer/TransferSingle/TransferBatch 事件）
- [ ] NFT Token 页：图片/metadata 展示
- [ ] 地址 NFT 持有列表
- [ ] NFT 转账历史

### 3.3 Token Indexer 扩展
- [ ] 索引 ERC-721 Transfer 事件
- [ ] 索引 ERC-1155 TransferSingle / TransferBatch
- [ ] token_balances 表：实时余额快照（替代从 transfers 聚合计算）
- [ ] 定期快照 + 增量更新
...
- [ ] 合约创建者 + 创建交易链接
- [ ] Similar Contracts 推荐（相同 bytecode hash）
...
- [ ] Top Token Holders（per token）
- [ ] Most Active Addresses（by tx count）
